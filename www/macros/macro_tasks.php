...
function macro_tasks($args) {
  ...
  if (!Identity::mayViewRoundTasks($round)) {
  ...
  $scores = getattr($args, 'score') && Identity::mayViewRoundScores($round);
  $filter_user = null;
  if (Identity::isAnonymous() && $scores && request('user', '')) {
  ...
  } else if (Identity::isAnonymous() || $scores == false) {
    $user_id = null;
    $filter_user_id = null;
  } else {
    $user_id = Identity::getId();
    $filter_username = request('user', Identity::getUsername());
  ...
  if ($filter_user_id
      && $display_tabs == 'true'
      && Identity::mayViewRoundScores($round)) {
    $tabs = task_list_tabs(
      $round['page_name'],
      $filter,
  ...
  $options['css_class'] = 'alternating-colors tasks table-sort';
