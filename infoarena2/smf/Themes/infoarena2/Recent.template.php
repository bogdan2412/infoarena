...
function template_unread()
{
	...
	if ($settings['show_mark_read'])
	{
		...
		$mark_read = array(
			'markread' => array('text' => 452, 'image' => 'markread.gif', 'lang' => true, 'url' => $scripturl . '?action=markasread;sa=' . (empty($context['current_board']) ? 'all' : 'board;board=' . $context['current_board'] . '.0') . ';sesc=' . $context['session_id']),
		);
	...
	if (!empty($context['topics']) && !$context['showing_all_topics'])
			echo '
				<tr class="titlebg">
					<td colspan="7" align="right" class="middletext"><a href="', $scripturl, '?action=unread;all', !empty($context['current_board']) ? ';board=' . $context['current_board'] . '.0' : '', '">', $txt['unread_topics_all'], '</a></td>
				</tr>';
